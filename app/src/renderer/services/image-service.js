...
class ImageService {
  ...
  async generateImage(prompt, provider = "gemini") {
    if (this.isGenerating) return;

    this.isGenerating = true;
    try {
      const settings = Storage.get(CONFIG.LOCAL_STORAGE_KEYS.SETTINGS, {});
  ...
  showGeneratedImage(imageBase64, filename) {
  ...
  async generateImageWithOpenAI(prompt, apiKey) {
    const response = await fetch(
